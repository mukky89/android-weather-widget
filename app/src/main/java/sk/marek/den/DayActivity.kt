        ...
        Surface(Modifier.fillMaxSize()) {
            LazyColumn(modifier = Modifier.safeDrawingPadding(), contentPadding = PaddingValues(22.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                item { Card(shape = RoundedCornerShape(20.dp)) { Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                    ...
                    Text("Vo widgete: vľavo otvoríš zobrazenú udalosť, vpravo dátum a Celý deň otvoria deň v kalendári. Počítadlo zahŕňa všetky udalosti daného dňa. Ak ich je viac, striedajú sa každých 8 sekúnd; číslo pri názve kalendára ukazuje poradie. Šípkami pri názve kalendára prepneš na predchádzajúcu alebo ďalšiu udalosť. Pravá časť aj + otvoria Google Kalendár alebo Outlook podľa riadku.", fontSize = 12.sp)
                ...
                item { Text("Meniny dnes: ${nameday(this@DayActivity)}", fontWeight = FontWeight.SemiBold)
                    ...
                    Text("Deň 0.14 · vytvorené pre Mareka", fontSize = 11.sp, modifier = Modifier.padding(top = 12.dp)) }
            }
        }
    }
}
